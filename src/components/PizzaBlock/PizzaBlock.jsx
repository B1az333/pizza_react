            ...
            <div className="pizza-block__selector">
                <ul>
                    {typeNames.map((typeName, index) => (
                        <li onClick={() => onSelectType(index)} className={classNames({ active: activeType === index, disabled: !types.includes(index) })} key={`${typeName}_${index}`}>
                            {typeName}
                        </li>
                    ))}
                </ul>
                <ul>
                    {allSizes.map((size, index) => (
                        <li onClick={() => onSelectSize(size)} className={classNames({ active: activeSize === size, disabled: !sizes.includes(size) })} key={`${size}_${index}`}>
                            {size} см.
                        </li>
                    ))}
                </ul>
